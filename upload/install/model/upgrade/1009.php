...
class ModelUpgrade1009 extends Model {
	public function upgrade() {
		...
		// Config - Information Subscription ID
		$query = $this->db->query("SELECT * FROM `setting` WHERE `key` = 'config_information_subscription_id'");

		if (!$query->num_rows) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "setting` SET `store_id` = '0', `code` = 'config', `key` = 'config_information_subscription_id', `value` = '7', `serialized` = '0'");
		}
