	...
	public function check(array $order_info): int {
		...
		if ($query->num_rows) {
			...
			*/
			...
			$request .= '&domain=' . urlencode(oc_substr(strrchr($order_info['email'], '@'), 1));
			...
			$request .= '&emailMD5=' . urlencode(md5(oc_strtolower($order_info['email'])));
		...
		if ($risk_score > $this->config->get('fraud_maxmind_score') && $this->config->get('fraud_maxmind_key')) {
			return (int)$this->config->get('fraud_maxmind_order_status_id');
		} else {
			return 0;
		}
	}
