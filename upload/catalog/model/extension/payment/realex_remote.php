...
class ModelExtensionPaymentRealexRemote extends Model {
	...
	public function capturePayment(string $account, float $amount, string $currency, int $order_id, string $order_ref, int $card_number, int $expire, string $name, string $type, int $cvv, string $issue, string $eci_ref, $eci = '', $cavv = '', $xid = ''): false|SimpleXMLElement {
		...
		if ($response->result == '00') {
			...
			// Referal B
			$this->model_checkout_order->addHistory($order_id, $this->config->get('payment_realex_remote_declined_pending_status_id'), $message);
		} elseif ($response->result == '103') {
			// Referal A
			$this->model_checkout_order->addHistory($order_id, $this->config->get('payment_realex_remote_declined_stolen_status_id'), $message);
		} elseif ($response->result == '200') {
			// Error Connecting to Bank
			$this->model_checkout_order->addHistory($order_id, $this->config->get('payment_realex_remote_declined_bank_status_id'), $message);
