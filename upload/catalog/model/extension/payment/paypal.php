...
class ModelExtensionPaymentPayPal extends Model {
	...
	public function hasProductInCart($product_id, $option = [], $subscription_plan_id = 0) {
	...
	public function addPayPalCustomerToken($data): void {
	...
	public function deletePayPalCustomerToken($customer_id, $payment_method, $vault_id): void {
	...
	public function setPayPalCustomerMainToken($customer_id, $payment_method, $vault_id): void {
	...
	public function addPayPalOrder($data): void {
	...
	public function editPayPalOrder($data): void {
	...
	public function deletePayPalOrder($order_id): void {
	...
	public function addPayPalOrderRecurring($data): void {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "paypal_checkout_integration_subscription` SET `subscription_id` = '" . (int)$data['subscription_id'] . "', `order_id` = '" . (int)$data['order_id'] . "', `date_added` = NOW(), `date_modified` = NOW(), `next_payment` = NOW(), `trial_end` = '" . $data['trial_end'] . "', `subscription_end` = '" . $data['subscription_end'] . "', `currency_code` = '" . $this->db->escape($data['currency_code']) . "', `total` = '" . $this->currency->format($data['amount'], $data['currency_code'], false, false) . "'");
	}

	public function editPayPalOrderRecurringNextPayment($subscription_id, $next_payment): void {
		$this->db->query("UPDATE `" . DB_PREFIX . "paypal_checkout_integration_subscription` SET `next_payment` = '" . $next_payment . "', `date_modified` = NOW() WHERE `subscription_id` = '" . (int)$subscription_id . "'");
	}

	public function deletePayPalOrderRecurring($order_id): void {
		$query = $this->db->query("DELETE FROM `" . DB_PREFIX . "paypal_checkout_integration_subscription` WHERE `order_id` = '" . (int)$order_id . "'");
	}

	public function getPayPalOrderRecurring($subscription_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "paypal_checkout_integration_subscription` WHERE `subscription_id` = '" . (int)$subscription_id . "'");

		return $query->row;
	}

	public function addOrderRecurring($order_id, $description, $data, $reference) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "subscription` SET `order_id` = '" . (int)$order_id . "', `date_added` = NOW(), `product_id` = '" . (int)$data['product_id'] . "', `product_name` = '" . $this->db->escape($data['name']) . "', `product_quantity` = '" . $this->db->escape($data['quantity']) . "', `recurring_id` = '" . (int)$data['subscription']['recurring_id'] . "', `recurring_name` = '" . $this->db->escape($data['name']) . "', `recurring_description` = '" . $this->db->escape($description) . "', `recurring_frequency` = '" . $this->db->escape($data['subscription']['frequency']) . "', `recurring_cycle` = '" . (int)$data['subscription']['cycle'] . "', `recurring_duration` = '" . (int)$data['subscription']['duration'] . "', `recurring_price` = '" . (float)$data['subscription']['price'] . "', `trial` = '" . (int)$data['subscription']['trial'] . "', `trial_frequency` = '" . $this->db->escape($data['subscription']['trial_frequency']) . "', `trial_cycle` = '" . (int)$data['subscription']['trial_cycle'] . "', `trial_duration` = '" . (int)$data['subscription']['trial_duration'] . "', `trial_price` = '" . (float)$data['subscription']['trial_price'] . "', `reference` = '" . $this->db->escape($reference) . "'");

		return $this->db->getLastId();
	}

	public function deleteOrderRecurring($order_id): void {
		...
		foreach ($query->rows as $order_recurring) {
			$this->deleteOrderRecurringTransaction($order_recurring['subscription_id']);
		}

		$query = $this->db->query("DELETE FROM `" . DB_PREFIX . "subscription` WHERE `order_id` = '" . (int)$order_id . "'");
	}
	...
	public function getOrderRecurrings() {
		...
		foreach ($query->rows as $order_recurring) {
			$order_recurring_data[] = $this->getOrderRecurring($order_recurring['subscription_id']);
		}

		return $order_recurring_data;
	}

	public function getOrderRecurring($subscription_id) {
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "subscription` WHERE `subscription_id` = '" . (int)$subscription_id . "'");

		return $query->row;
	}

	public function addOrderRecurringTransaction($data): void {
	...
	public function deleteOrderRecurringTransaction($order_recurring_id): void {
		$query = $this->db->query("DELETE FROM `" . DB_PREFIX . "order_recurring_transaction` WHERE `order_recurring_id` = '" . (int)$order_recurring_id . "'");
	}

	public function subscriptionPayment($product_data, $order_data, $paypal_order_data): void {
		$_config = new Config();
		$_config->load('paypal');
		...
		$recurring_name = $product_data['subscription']['name'];

		if ($product_data['subscription']['trial'] == 1) {
			$price = $product_data['subscription']['trial_price'];
			$trial_amt = $this->currency->format($this->tax->calculate($product_data['subscription']['trial_price'], $product_data['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency'], false, false) * $product_data['quantity'] . ' ' . $this->session->data['currency'];
			$trial_text = sprintf($this->language->get('text_trial'), $trial_amt, $product_data['subscription']['trial_cycle'], $product_data['subscription']['trial_frequency'], $product_data['subscription']['trial_duration']);
		} else {
			$price = $product_data['subscription']['price'];
			$trial_text = '';
		}

		$recurring_amt = $this->currency->format($this->tax->calculate($product_data['subscription']['price'], $product_data['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency'], false, false) * $product_data['quantity'] . ' ' . $this->session->data['currency'];
		$recurring_description = $trial_text . sprintf($this->language->get('text_recurring'), $recurring_amt, $product_data['subscription']['cycle'], $product_data['subscription']['frequency']);

		if ($product_data['subscription']['duration'] > 0) {
			$recurring_description .= sprintf($this->language->get('text_length'), $product_data['subscription']['duration']);
		}

		$subscription_id = $this->addOrderRecurring($order_data['order_id'], $recurring_description, $product_data, $paypal_order_data['transaction_id']);
		...
		if (!empty($paypal_order_data['vault_id'])) {
			...
			if (($product_data['subscription']['trial'] == 1) && ($product_data['subscription']['trial_duration'] != 0)) {
				$next_payment = $this->calculateSchedule($product_data['subscription']['trial_frequency'], $next_payment, $product_data['subscription']['trial_cycle']);
				$trial_end = $this->calculateSchedule($product_data['subscription']['trial_frequency'], $trial_end, $product_data['subscription']['trial_cycle'] * $product_data['subscription']['trial_duration']);
			} elseif ($product_data['subscription']['trial'] == 1) {
				$next_payment = $this->calculateSchedule($product_data['subscription']['trial_frequency'], $next_payment, $product_data['subscription']['trial_cycle']);
				$trial_end = new DateTime('0000-00-00');
			}

			if (date_format($trial_end, 'Y-m-d H:i:s') > date_format($subscription_end, 'Y-m-d H:i:s') && $product_data['subscription']['duration'] != 0) {
				$subscription_end = new DateTime(date_format($trial_end, 'Y-m-d H:i:s'));
				$subscription_end = $this->calculateSchedule($product_data['subscription']['frequency'], $subscription_end, $product_data['subscription']['cycle'] * $product_data['subscription']['duration']);
			} elseif (date_format($trial_end, 'Y-m-d H:i:s') == date_format($subscription_end, 'Y-m-d H:i:s') && $product_data['subscription']['duration'] != 0) {
				$next_payment = $this->calculateSchedule($product_data['subscription']['frequency'], $next_payment, $product_data['subscription']['cycle']);
				$subscription_end = $this->calculateSchedule($product_data['subscription']['frequency'], $subscription_end, $product_data['subscription']['cycle'] * $product_data['subscription']['duration']);
			} elseif (date_format($trial_end, 'Y-m-d H:i:s') > date_format($subscription_end, 'Y-m-d H:i:s') && $product_data['subscription']['duration'] == 0) {
			...
			$result = $this->createPayment($order_data, $paypal_order_data, $price, $subscription_id, $recurring_name);
	...
	public function cronPayment(): void {
		...
		foreach ($order_recurrings as $order_recurring) {
			if ($order_recurring['status'] == 1) {
				$paypal_order_recurring = $this->getPayPalOrderRecurring($order_recurring['subscription_id']);
				...
				if ($paypal_order_recurring) {
					...
					if (!empty($paypal_order_info['vault_id'])) {
						...
						if ($transaction_id && $transaction_status && $currency_code && $amount) {
							if (($transaction_status == 'CREATED') || ($transaction_status == 'COMPLETED') || ($transaction_status == 'PENDING')) {
								$order_recurring_transaction_data = [
									'subscription_id' => $order_recurring['subscription_id'],
									'reference'       => $transaction_id,
									'type'            => '1',
									'amount'          => $amount
								];
								...
								$this->editPayPalOrderRecurringNextPayment($order_recurring['subscription_id'], date_format($next_payment, 'Y-m-d H:i:s'));
							} else {
								$order_recurring_transaction_data = [
									'subscription_id' => $order_recurring['subscription_id'],
									'reference'       => $transaction_id,
	...
	public function createPayment($order_data, $paypal_order_data, $price, $subscription_id, $recurring_name) {
		$this->load->language('extension/payment/paypal');
	...
	public function calculateSchedule($frequency, \DateTime $next_payment, $cycle) {
	...
	public function log($data, $title = ''): void {
		// Setting
		$_config = new Config();
		$_config->load('paypal');
