...
class ModelExtensionPaymentSagePayDirect extends Model {
	public function getMethod($address, $total) {
		...
		if ($this->config->get('payment_sagepay_direct_total') > 0 && $this->config->get('payment_sagepay_direct_total') > $total) {
			$status = false;
	...
	private function setPaymentData($order_info, $sagepay_order_info, $price, $subscription_id, $recurring_name, $i = null) {
