                            ...
                            if ($order_product && $order_product['order_product_id'] == $subscription['order_product_id']) {
                                ...
                                foreach ($products as $product) {
                                    if ($product['product_id'] == $order_product['product_id']) {
                                        $trial_price = $this->currency->format($this->tax->calculate($result['trial_price'], $product['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
                                        ...
                                        $price = $this->currency->format($this->tax->calculate($result['price'], $product['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
                                        $cycle = $result['cycle'];
                                                ...
                                                if (isset($this->{'model_extension_payment_' . $payment_method['code']}->promotion)) {
                                                    ...
                                                    */
                                                    ...
                                                    if ($subscription_active_status_id == $subscription_status_id) {
                                                                                    ...
                                                                                    if ($next_date_cycle >= 0 && $next_order_info['order_status_id'] == $this->config->get('config_subscription_pending_status_id')) {
                                                                                        ...
                                                                                        if ($product_subscription_info && (int)$product_subscription_info['cycle'] >= 0 && $product_subscription_info['duration'] == $next_subscription['duration']) {
                                                                                            if ($product_subscription_info['subscription_plan_id'] == $next_subscription['subscription_plan_id']) {
                                                                                                // Add Transaction
                                                                                                $this->model_account_subscription->addTransaction($subscription_id, $next_subscription['order_id'], $language->get('text_promotion'), $next_subscription['amount'], $next_subscription['type'], $next_order_info['payment_method'], $next_order_info['payment_code']);
                                                                                            } else {
                                                                                                $this->model_checkout_subscription->addHistory($next_subscription['order_id'], $subscription_status_id, $language->get('error_subscription_plan'));
                                                                                            }
                                                                                        }
                                                                                    } else {
                                                                                        $this->model_checkout_subscription->addHistory($next_subscription['order_id'], $subscription_status_id, $language->get('error_next_order'));
                                                                                    }
                                                                                }
                                                                            }
                                                                        } else {
                                                                            $this->model_checkout_subscription->addHistory($next_subscription['order_id'], $subscription_status_id, $language->get('error_next_subscription'));
                                                                        }
                                                                    }
                                                                }
                                                            }
                                                        ...
                                                        $this->model_checkout_subscription->addHistory($order_info['order_id'], $subscription_status_id, $language->get('error_subscription_status'));
                                                    }
                                                }
                                            }
                                ...
                                if (isset($this->{'model_extension_payment_' . $payment_method['code']}->cancel)) {
                                    ...
                                    if ($subscription_status_id == $this->config->get('config_subscription_canceled_status_id')) {
                                        ...
                                        if ($subscription_info) {
                                            ...
                                            $this->model_account_subscription->addTransaction($subscription_id, $subscription['order_id'], $this->language->get('text_cancelled'), $subscription_info['amount'], $subscription_info['type'], $order_info['payment_method'], $order_info['payment_code']);
