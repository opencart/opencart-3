	...
	public function acsReturn(): void {
		if (isset($this->session->data['order_id'])) {
			...
			if ($signature_result->result == '00' && (strtoupper($signature_result->threedsecure->status) == 'Y' || strtoupper($signature_result->threedsecure->status) == 'A')) {
				...
				if ($this->config->get('payment_realex_remote_liability') != 1 && isset($cavv) && isset($eci_ref) && isset($xid)) {
					...
					$this->model_checkout_order->addHistory($md['order_id'], $this->config->get('payment_realex_remote_declined_status_id'), $message);
			...
			if (isset($cavv) && isset($eci_ref) && isset($xid)) {
				$capture_result = $this->model_extension_payment_realex_remote->capturePayment($md['account'], $md['amount'], $md['currency'], $md['order_id'], $md['order_ref'], $md['cc_number'], $md['cc_expire'], $md['cc_name'], $md['cc_type'], $md['cc_cvv2'], $md['cc_issue'], $eci_ref, $eci, $cavv, $xid);
