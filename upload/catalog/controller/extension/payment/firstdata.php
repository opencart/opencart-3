					...
					if (isset($this->request->post['approval_code'])) {
						...
						if ($response_parts[0] == 'Y') {
							...
							if ($this->config->get('payment_firstdata_auto_settle') == 1) {
								...
								$this->model_checkout_order->addHistory($order_id, $this->config->get('payment_firstdata_success_settled_status_id'), $message, false);
							} else {
								$this->model_extension_payment_firstdata->addTransaction($fd_order_id, 'auth', []);

								$this->model_checkout_order->addHistory($order_id, $this->config->get('payment_firstdata_success_unsettled_status_id'), $message, false);
							}
						} else {
