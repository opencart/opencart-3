				...
				if (!$error) {
					...
					if ($order_info) {
						...
						$post_data = [
							...
							'capture'    => $capture,
							'bankaba'    => $bankaba,
							'userfields' => ['secret_token' => $this->config->get('payment_cardconnect_token')],
							'frontendid' => '26'
						];
						...
						if (isset($response_data['respstat']) && $response_data['respstat'] == 'A') {
							...
							$this->model_extension_payment_cardconnect->addTransaction($cardconnect_order_id, $type, $order_info['retref'], $order_info['total'], $order_info['currency_code']);
	...
	private function validate(): array {
		...
		if (!isset($this->request->post['method']) || $this->request->post['method'] == 'card') {
			if ($this->request->post['card_new']) {
				if (!isset($this->request->post['card_number']) || oc_strlen($this->request->post['card_number']) < 1 || oc_strlen($this->request->post['card_number']) > 19) {
					$error['card_number'] = $this->language->get('error_card_number');
				}

				if (!isset($this->request->post['card_cvv2']) || oc_strlen($this->request->post['card_cvv2']) < 1 || oc_strlen($this->request->post['card_cvv2']) > 4) {
			...
			if ($this->config->get('payment_cardconnect_echeck')) {
				if (!isset($this->request->post['account_number']) || oc_strlen($this->request->post['account_number']) < 1 || oc_strlen($this->request->post['account_number']) > 19) {
					$error['account_number'] = $this->language->get('error_account_number');
				}

				if (!isset($this->request->post['routing_number']) || oc_strlen($this->request->post['routing_number']) < 1 || oc_strlen($this->request->post['routing_number']) > 9) {
