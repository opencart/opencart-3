	...
	public function release(): void {
		...
		if (!empty($this->request->post['order_id']) && !empty($this->request->post['amount']) && $this->request->post['amount'] > 0) {
			...
			if (!empty($release_response) && $release_response['Status'] == 'OK') {
				...
				$json['msg'] = isset($release_response['StatusDetail']) ? (string)$release_response['StatusDetail'] : 'Unable to release';
			}
		} else {
			$json['error'] = true;
	...
	public function rebate(): void {
		...
		if (!empty($this->request->post['order_id'])) {
			...
			if (!empty($rebate_response) && $rebate_response['Status'] == 'OK') {
				...
				$json['msg'] = isset($rebate_response['StatusDetail']) ? (string)$rebate_response['StatusDetail'] : 'Unable to rebate';
			}
		} else {
			$json['error'] = true;
			$json['msg'] = 'Missing data';
