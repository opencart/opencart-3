...
class ControllerExtensionPaymentSquareup extends Controller {
    ...
    public function transaction_info(): void {
        ...
        $data['url_transaction'] = sprintf(Squareup::VIEW_TRANSACTION_URL, $transaction_info['transaction_id'], $transaction_info['location_id']);
        ...
        if ($data['has_refunds']) {
            ...
            foreach ($refunds as $refund) {
                $amount = $this->currency->format($this->squareup->standardDenomination($refund['amount_money']['amount'], $refund['amount_money']['currency']), $refund['amount_money']['currency']);

                $fee = $this->currency->format($this->squareup->standardDenomination($refund['processing_fee_money']['amount'], $refund['processing_fee_money']['currency']), $refund['processing_fee_money']['currency']);
    ...
    public function oauth_callback() {
        ...
        try {
            ...
            if (!isset($previous_setting['payment_squareup_location_id']) || (isset($previous_setting['payment_squareup_location_id']) && !in_array($previous_setting['payment_squareup_location_id'],
    ...
    public function refund(): void {
        $this->transactionAction(function($transaction_info, &$json) {
            ...
            if ($last_refund) {
                $refunded_amount = $this->currency->format($this->squareup->standardDenomination($last_refund['amount_money']['amount'], $last_refund['amount_money']['currency']), $last_refund['amount_money']['currency']);
