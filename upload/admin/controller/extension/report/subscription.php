 ...
 */
class ControllerExtensionReportSubscription extends Controller {
	/**
	...
	public function index(): void {
		$this->load->language('extension/report/subscription');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/report/subscription', 'user_token=' . $this->session->data['user_token'], true)
		];

		$data['action'] = $this->url->link('extension/report/subscription', 'user_token=' . $this->session->data['user_token'], true);
		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=report', true);

		if (isset($this->request->post['report_subscription_status'])) {
			$data['report_subscription_status'] = $this->request->post['report_subscription_status'];
		} else {
			$data['report_subscription_status'] = $this->config->get('report_subscription_status');
		}

		if (isset($this->request->post['report_subscription_sort_order'])) {
			$data['report_subscription_sort_order'] = $this->request->post['report_subscription_sort_order'];
		} else {
		...
		$this->response->setOutput($this->load->view('extension/report/subscription_form', $data));
	}
	...
	protected function validate(): bool {
		if (!$this->user->hasPermission('modify', 'extension/report/subscription')) {
	...
	public function report(): string {
		...
		// Subscriptions
		$this->load->model('extension/report/subscription');
		...
		$subscription_total = $this->model_extension_report_subscription->getTotalSubscriptions($filter_data);

		$results = $this->model_extension_report_subscription->getSubscriptions($filter_data);
		...
		$pagination->url = $this->url->link('report/report', 'user_token=' . $this->session->data['user_token'] . '&code=subscription' . $url . '&page={page}', true);
		...
		return $this->load->view('extension/report/subscription_info', $data);
	}
}
