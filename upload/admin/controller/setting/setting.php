...
class ControllerSettingSetting extends Controller {
	...
	public function index(): void {
		...
		if (isset($this->request->post['config_telephone_required'])) {
			...
			$data['config_telephone_required'] = 0;
		}
	...
	protected function validate(): bool {
		...
		if ((oc_strlen($this->request->post['config_telephone']) < 3) || (strlen($this->request->post['config_telephone']) > 32)) {
		...
		if (!$this->request->post['config_limit_autocomplete']) {
			$this->error['limit_autocomplete'] = $this->language->get('error_limit_autocomplete');
		}
