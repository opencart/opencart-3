...
class ModelExtensionDashboardSale extends Model {
    public function getTotalSales(array $data = array()): float {
        ...
        return isset($query->row['total']) ? (float)$query->row['total'] : 0.00;
    }
    ...
    public function getTotalOrdersByDay(): array {
        ...
        foreach ($query->rows as $result) {
            $order_data[$result['hour']] = array(
                'hour' => $result['hour'], 'total' => $result['total']);
        }
    ...
    public function getTotalOrdersByWeek(): array {
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            $order_data[date('w', strtotime($date))] = array(
                'day' => date('D', strtotime($date)), 'total' => 0);
        }
        ...
        foreach ($query->rows as $result) {
            $order_data[date('w', strtotime($result['date_added']))] = array(
                'day' => date('D', strtotime($result['date_added'])), 'total' => $result['total']);
        }
    ...
    public function getTotalOrdersByYear(): array {
        ...
        for ($i = 1; $i <= 12; $i++) {
            $order_data[$i] = array(
                'month' => date('M', mktime(0, 0, 0, $i)), 'total' => 0);
        }
        ...
        foreach ($query->rows as $result) {
            $order_data[date('n', strtotime($result['date_added']))] = array(
                'month' => date('M', strtotime($result['date_added'])), 'total' => $result['total']);
        }
    ...
    public function getTaxes(array $data = array()): array {
        ...
        switch ($group) {
            ...
            case 'week':
                ...
                break;
            case 'month':
                $sql .= " GROUP BY YEAR(o.`date_added`), MONTH(o.`date_added`), ot.`titl`e";
                break;
            case 'year':
